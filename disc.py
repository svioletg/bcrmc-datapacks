...
def main() -> int | None:
    ...
    if (fp := (DATA_PATH / 'jukebox_song' / f'{name}.json')).is_file():
        ...
        fp.touch()
        print(f'Created: {fp}')
        fp.write_text(TMPL_DATA_JUKEBOX_SONG.format(name=name))
    ...
    if (fp := (DATA_PATH / 'recipe' / f'{mdname}.json')).is_file():
        ...
        fp.touch()
        print(f'Created: {fp}')
        fp.write_text(TMPL_DATA_RECIPE.format(name=name))
    ...
    if (fp := (RES_PATH / 'items' / f'{mdname}.json')).is_file():
        ...
        fp.touch()
        print(f'Created: {fp}')
        fp.write_text(TMPL_RES_ITEM.format(name=name))
    ...
    if not any(i['model'] == f'bcrmc7:item/music_disc_{name}' for i in discmodel['overrides']):
        discmodel['overrides'].append(
            {'predicates': {'custom_model_data': highest + 1}, 'model': f'bcrmc7:item/music_disc_{name}'}
        )
        with open(RES_PATH / 'models/item/music_disc_far.json', 'w', encoding='utf-8') as f:
            json.dump(discmodel, f)

    with open(RES_PATH / 'sounds.json', 'r', encoding='utf-8') as f:
        sounds: dict[str, Any] = json.load(f)

    if f'music_disc.{name}' in sounds:
        sounds[f'music_disc.{name}'] = {'sounds': [{'name': f'bcrmc7:records/{name}'}]}
